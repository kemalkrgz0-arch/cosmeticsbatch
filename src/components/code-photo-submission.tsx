    ...
    <section className="mt-8 rounded-2xl border border-border bg-card p-5 shadow-card sm:p-6">
      ...
      {open && (
        <form onSubmit={submit} className="mt-5 space-y-4 border-t border-border pt-5">
          ...
          <label className="block text-sm font-medium">Email for our reply<input type="email" required autoComplete="email" value={email} maxLength={254} onChange={(e) => setEmail(e.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-border bg-bg px-3 outline-none focus:border-accent" /></label>
          <label className="flex items-start gap-2.5 text-sm text-fg-muted"><input type="checkbox" required checked={consent} onChange={(e) => setConsent(e.target.checked)} className="mt-1" /><span>I confirm this is my photo and allow Cosmetics Batch to store the photo and my email, contact me about this submission, and use verified photo portions to improve its public guides. Personal details should not be visible.</span></label>
          ...
          <button disabled={!file || !email || !consent || state === "sending"} className="inline-flex h-11 items-center gap-2 rounded-xl bg-cta px-5 text-sm font-semibold text-cta-fg disabled:cursor-not-allowed disabled:opacity-50">{state === "sending" && <LoaderCircle className="h-4 w-4 animate-spin" />} Send for review</button>
