...
export function MobileHeaderMenu({ direction, items, label, openLabel, closeLabel }: { direction: "ltr" | "rtl"; items: { href: string; label: string }[]; label: string; openLabel: string; closeLabel: string }) {
  ...
  return (
    <>
      ...
      {open && <div dir={direction} className="fixed inset-0 z-[70] lg:hidden" role="dialog" aria-modal="true" aria-label={label}>
        ...
        <div id="mobile-site-menu" ref={panel} className="absolute inset-x-3 top-3 rounded-[1.7rem] border border-border bg-card p-4 shadow-card">
          <div dir="ltr" className="flex justify-end"><button type="button" aria-label={closeLabel} onClick={() => { setOpen(false); trigger.current?.focus(); }} className="inline-flex size-11 items-center justify-center rounded-full hover:bg-bg-subtle"><X className="size-6" aria-hidden="true" /></button></div>
